...
项目参考了 LumenX Studio 的内容分析与故事资产提取思路，并针对多章节小说改编进一步增强了全书规划、逐章并行生成、渐进式预览、剧本媒介差异、原文追溯、结构校验和作者编辑闭环。

## 原创实现与第三方引用

本项目的 Java 后端、模型接入、全书改编蓝图、分章并行生成、响应归一化、异步任务、YAML Schema、语义校验、渐进式预览和浏览器编辑器均为本项目实现。项目未复制 LumenX 源码，详细的参考与改进说明见 `docs/LUMENX_REFERENCE.md`。
