...
生成流程先提取人物、地点和道具，再生成覆盖全部章节的改编蓝图，最后把 Story Bible 与蓝图共同作为受约束输入交给逐章剧本生成阶段。全书状态约束、逐章并行精编和跨章节连续性检查可以降低人物改名、地点漂移、关键道具丢失以及情节重复的问题。
...
- 使用全书蓝图、场景级媒介结构、原文证据和跨引用校验增强小说改编结果的可控性。
